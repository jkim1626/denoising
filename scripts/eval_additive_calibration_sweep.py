...
def main():
    ...
    use_module = os.path.exists(script_path)
    ...
    for c in args.scales:
        ...
        for f in files:
            ...
            pin.append(psnr_db(clean, noisy))
            pout.append(psnr_db(clean, den))

        if mse_d:
            dsnr = st.mean([o - i for i, o in zip(sin, sout)])
            dpsnr = st.mean([o - i for i, o in zip(pin, pout)])
            print(f"[scale {c:.2f}] Files={len(mse_d)}  ΔSNR={dsnr:.2f} dB  ΔPSNR={dpsnr:.2f} dB  "
